...
describe('Manipulator', () => {
  ...
  describe('setDataAttribute', () => {
    ...
    it('should set data attribute in kebab case', () => {
      ...
      expect(div.getAttribute('data-bs-test.html-key')).toEqual('value')
    })
  })
  ...
  describe('removeDataAttribute', () => {
    ...
    it('should remove data attribute in kebab case', () => {
      fixtureEl.innerHTML = '<div data-bs-test.html-key="value"></div>'

      const div = fixtureEl.querySelector('div')

      Manipulator.removeDataAttribute(div, 'testKey')
      expect(div.getAttribute('data-bs-test.html-key')).toBeNull()
    })
  })
  ...
  describe('getDataAttribute', () => {
    it('should only get bs-prefixed data attribute', () => {
      fixtureEl.innerHTML = '<div data-bs-key="value" data-test.html-bs="postFixed" data-toggle="tab"></div>'
    ...
    it('should normalize data', () => {
      fixtureEl.innerHTML = '<div data-bs-test.html="false" ></div>'

      const div = fixtureEl.querySelector('div')

      expect(Manipulator.getDataAttribute(div, 'test')).toBeFalse()

      div.setAttribute('data-bs-test.html', 'true')
      expect(Manipulator.getDataAttribute(div, 'test')).toBeTrue()

      div.setAttribute('data-bs-test.html', '1')
      expect(Manipulator.getDataAttribute(div, 'test')).toEqual(1)
    })

    it('should normalize json data', () => {
      fixtureEl.innerHTML = '<div data-bs-test.html=\'{"delay":{"show":100,"hide":10}}\'></div>'
      ...
      div.setAttribute('data-bs-test.html', encodeURIComponent(dataStr))
      expect(Manipulator.getDataAttribute(div, 'test')).toEqual(objectData)

      div.setAttribute('data-bs-test.html', dataStr)
      expect(Manipulator.getDataAttribute(div, 'test')).toEqual(objectData)
    })
  })
})
